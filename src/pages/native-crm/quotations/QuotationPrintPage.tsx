...
export default function QuotationPrintPage() {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const res = await api.get(`/api/v1/native-crm/pdf/quotations/${id}?template=${variant}`, { responseType: 'blob' });
      ...
      <div className={`w-[210mm] min-h-[297mm] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none p-[15mm] font-sans text-sm text-gray-800 ${a4Cls}${editing ? ' outline outline-2 outline-brand-400 outline-offset-4' : ''}`}>
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="text-right">
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">QUOTATION</h1>
            <p className="text-gray-400 text-xs mt-1">{item.quotationId}</p>
            <div className="text-xs text-gray-600 space-y-0.5 mt-2">
              <div><span className="text-gray-400">Date:</span> {fmtD(item.createdAt)}</div>
              {item.validUntil && <div><span className="text-gray-400">Valid Until:</span> {fmtD(item.validUntil)}</div>}
