...
import { useQuotationQuery, useQuotationUpdate } from '../../../modules/native-crm/queries/quotations.queries';
...
export default function QuotationPrintPage() {
  ...
  const { data: designerTemplates = [] } = useCustomTemplatesQuery('quotation');
  ...
  const handleShare = async () => {
    ...
    setSharing(true);
    try {
      const res = await api.post('/api/v1/portal/generate-token', { docType: 'quotation', docId: item._id });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/v1/native-crm/pdf/quotations/${id}?${qs}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
      const a   = document.createElement('a');
      a.href = url; a.download = `quotation-${item?.quotationId ?? id}.pdf`;
  ...
  if (!item) return <div className="flex items-center justify-center h-screen text-gray-500">Quotation not found.</div>;
  ...
  const tdCls = variant === 'minimal' ? 'border-b border-gray-100 px-3 py-2'
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      {tplId && id ? (
        <TemplateDocViewer ref={viewerRef} module="quotations" docId={id} templateId={tplId} />
      ) : (
      <div className={`w-[210mm] min-h-[297mm] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none ${isModern ? '' : 'p-[15mm]'} ${pageFontCls} text-sm text-gray-800 ${a4Cls}${editing ? ' outline outline-2 outline-brand-400 outline-offset-4' : ''}`}>
        ...
        <div className={headerWrapCls}>
          ...
          <div className="text-right">
            <h1 className={docTitleCls}>QUOTATION</h1>
            <p className={docIdCls}>{item.quotationId}</p>
            <div className={docMetaCls}>
              <div><span className={metaLblCls}>Date:</span> {fmtD(item.createdAt)}</div>
              {item.validUntil && <div><span className={metaLblCls}>Valid Until:</span> {fmtD(item.validUntil)}</div>}
        ...
        <div className={infoRowCls}>
          ...
          <div className="text-xs text-gray-500 mt-1 space-y-0.5">
            ...
            {item.address && custAddr !== item.address && <div>{item.address}</div>}
          </div>
        </div>
        ...
        <table className="w-full border-collapse mb-2 text-xs" style={{ tableLayout: 'fixed' }}>
          ...
          <thead>
            <tr className={thCls}>
              ...
              <th className={`${tdCls} text-left font-semibold`}>Description</th>
        ...
        {settings?.quotationFooter && (
          <p className="text-xs text-gray-400 border-t border-gray-200 pt-3 mb-6 whitespace-pre-line text-center">{settings.quotationFooter}</p>
        )}
        ...
        <div className="flex justify-between mt-12 text-xs text-gray-500">
          <div className="text-center">
            ...
            <div className="border-t border-gray-400 w-44 pt-1.5">Authorised Signature</div>
          </div>
          <div className="text-center">
