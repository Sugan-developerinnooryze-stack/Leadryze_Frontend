...
export default function WorkorderPrintPage() {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const res = await api.get(`/api/v1/native-crm/pdf/workorders/${id}?template=${variant}`, { responseType: 'blob' });
      ...
      <div className={`w-[210mm] min-h-[297mm] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none p-[15mm] font-sans text-sm text-gray-800 ${a4Cls}${editing ? ' outline outline-2 outline-brand-400 outline-offset-4' : ''}`}>
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-400 text-xs mt-1">{item.workOrderId}</p>
            <div className="text-xs text-gray-600 space-y-0.5 mt-2">
              ...
              {item.scheduledDate && <div><span className="text-gray-400">Scheduled:</span> {fmtD(item.scheduledDate)}</div>}
              {item.completedDate && <div><span className="text-gray-400">Completed:</span> {fmtD(item.completedDate)}</div>}
        ...
        {(item.siteId || item.teamId || item.staffId) && (
          <div className="grid grid-cols-2 gap-4 mb-6 text-xs">
            {item.siteId  && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Site</p><p className="font-medium text-gray-800">{item.siteId}</p></div>}
            {item.teamId  && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Team</p><p className="font-medium text-gray-800">{item.teamId}</p></div>}
            {item.staffId && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Assigned Staff</p><p className="font-medium text-gray-800">{item.staffId}</p></div>}
          </div>
        )}
