...
import { useWorkorderQuery, useWorkorderUpdate } from '../../../modules/native-crm/queries/workorders.queries';
...
export default function WorkorderPrintPage() {
  ...
  const { data: designerTemplates = [] } = useCustomTemplatesQuery('workorder');
  ...
  const { data: siteList }        = useSitesListQuery({ page: 1, limit: 500 });
  const { data: staffList }       = useStaffsListQuery({ page: 1, limit: 500 });
  const { data: teamList }        = useTeamsListQuery({ page: 1, limit: 500 });
  const updateMutation            = useWorkorderUpdate();

  const customer = custList?.items?.find((c: any) => c.customerId === item?.customerId) ?? null;
  const site  = siteList?.items?.find((s: any) => s.siteId === item?.siteId) ?? null;
  const staff = staffList?.items?.find((s: any) => s.staffId === item?.staffId) ?? null;
  const team  = teamList?.items?.find((t: any) => t.teamId === item?.teamId) ?? null;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/v1/native-crm/pdf/workorders/${id}?${qs}`, { responseType: 'blob' });
  ...
  if (!item) return <div className="flex items-center justify-center h-screen text-gray-500">Work order not found.</div>;
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="print:hidden bg-white border-b border-gray-200 shadow-sm">
        <div className="flex items-center gap-2 px-4 py-2 flex-wrap justify-end">
          ...
          {!editing && item && (() => {
            const STEP_LABELS: Record<string, string> = { invoice: 'Create Invoice', contract: 'Create Contract' };
            const STEP_PATHS: Record<string, string>  = { invoice: '/native-crm/invoices', contract: '/native-crm/contracts' };
      ...
      {tplId && id ? (
        <TemplateDocViewer ref={viewerRef} module="workorders" docId={id} templateId={tplId} />
      ) : (
      <div className={`w-[210mm] min-h-[297mm] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none ${isModern ? '' : 'p-[15mm]'} ${pageFontCls} text-sm text-gray-800 ${a4Cls}${editing ? ' outline outline-2 outline-brand-400 outline-offset-4' : ''}`}>
        ...
        <div className={headerWrapCls}>
          ...
          <div className="text-right">
            <h1 className={docTitleCls}>WORK ORDER</h1>
            <p className={docIdCls}>{item.workOrderId}</p>
            <div className={docMetaCls}>
              <div><span className={metaLblCls}>Created:</span> {fmtD(item.createdAt)}</div>
              {item.scheduledDate && <div><span className={metaLblCls}>Scheduled:</span> {fmtD(item.scheduledDate)}</div>}
              {item.completedDate && <div><span className={metaLblCls}>Completed:</span> {fmtD(item.completedDate)}</div>}
              <div className="flex justify-end gap-1.5 mt-1">
                <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${priorityColor}`}>{item.priority}</span>
                <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-600 capitalize">{item.status?.replace(/_/g, ' ')}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Details Grid */}
        {(item.siteId || item.teamId || item.staffId) && (
          <div className="grid grid-cols-2 gap-4 mb-6 text-xs">
            {item.siteId  && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Site</p><p className="font-medium text-gray-800">{site?.name ?? item.siteId}</p></div>}
            {item.teamId  && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Team</p><p className="font-medium text-gray-800">{team?.name ?? item.teamId}</p></div>}
            {item.staffId && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Assigned Staff</p><p className="font-medium text-gray-800">{staffName || item.staffId}</p></div>}
          </div>
        )}

        {/* Work Description */}
        <div className="bg-gray-50 rounded p-3 mb-6">
          <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Work Description</p>
          {editing
        ...
        {/* Footer */}
        {settings?.workorderFooter && (
          <p className="text-xs text-gray-400 border-t border-gray-200 pt-3 mb-6 whitespace-pre-line text-center">{settings.workorderFooter}</p>
        )}
        ...
        <div className="flex justify-between mt-12 text-xs text-gray-500">
          <div className="text-center">
            ...
            <div className="border-t border-gray-400 w-44 pt-1.5">Technician Signature</div>
          </div>
          <div className="text-center">
            <div className="h-14 mb-1" />
            <div className="border-t border-gray-400 w-44 pt-1.5">Customer Signature &amp; Date</div>
          </div>
        </div>
        </div>
      </div>
