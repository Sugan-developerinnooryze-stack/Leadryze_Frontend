...
export default function ContractPrintPage() {
  ...
  const { data: designerTemplates = [] } = useCustomTemplatesQuery('contract');
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/v1/native-crm/pdf/contracts/${id}?${qs}`, { responseType: 'blob' });
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      {tplId && id ? (
        <TemplateDocViewer ref={viewerRef} module="contracts" docId={id} templateId={tplId} />
      ) : (
      <div className={`w-[210mm] min-h-[297mm] mx-auto my-8 print:my-0 bg-white shadow-xl print:shadow-none ${isModern ? '' : 'p-[15mm]'} ${pageFontCls} text-sm text-gray-800 ${a4Cls}${editing ? ' outline outline-2 outline-brand-400 outline-offset-4' : ''}`}>
        ...
        <div className={headerWrapCls}>
          ...
          <div className="text-right">
            <h1 className={docTitleCls}>SERVICE CONTRACT</h1>
            <p className={docIdCls}>{item.contractId}</p>
            <div className={docMetaCls}>
              <div><span className={metaLblCls}>Created:</span> {fmtD(item.createdAt)}</div>
              {item.startDate && <div><span className={metaLblCls}>Start:</span> {fmtD(item.startDate)}</div>}
              {item.endDate   && <div><span className={metaLblCls}>End:</span>   {fmtD(item.endDate)}</div>}
        ...
        {/* Parties */}
        <div className={`grid grid-cols-2 gap-6 ${infoRowCls}`}>
          <div>
        ...
        {(item.siteId || item.teamId || item.staffId) && (
          <div className="grid grid-cols-3 gap-4 mb-6 text-xs">
            ...
            {item.staffId && <div><p className="text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Assigned To</p><p className="font-medium text-gray-800">{staffName || item.staffId}</p></div>}
          </div>
        )}
